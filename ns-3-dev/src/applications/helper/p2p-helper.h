...
public:
  /**
   * Create P2PServerHelper which will make life easier for people trying
  ...
  /**
   * Create a P2PServerApplication on the specified Node.
   *
  ...
  /**
   * Create a P2PServerApplication on specified node
   *
   ...
   *
   * Create one p2p server application on each of the Nodes in the
   * NodeContainer.
   *
...
private:
  /**
   * Install an ns3::P2PServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an P2PServer will be installed.
...
public:
  /**
   * Create P2PClientHelper which will make life easier for people trying
   ...
   *
   * \param ip The IP address of the remote p2p server
   * \param port The port number of the remote p2p server
   */
  P2PClientHelper (Address ip, uint16_t port);
  /**
   * Create P2PClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
   * include a port value (e.g., InetSocketAddress and Inet6SocketAddress).
   *
   * \param addr The address of the remote p2p server
   */
  P2PClientHelper (Address addr);
  ...
  /**
   * Given a pointer to a P2PClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be P2PClient).
  ...
  /**
   * Create a p2p client application on the specified node.  The Node
   * is provided as a Ptr<Node>.
   *
   * \param node The Ptr<Node> on which to create the P2PClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the P2PClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
   ...
   * \param c the nodes
   *
   * Create one p2p client application on each of the input nodes
   *
...
private:
  /**
   * Install an ns3::P2PClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an P2PClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
